...
function M.toggle_diagnostic_virtual_text(bufnr, toggle_state)
  ...
  toggle_state.diagnostic_virtual_lines = toggle_state.diagnostic_virtual_lines and false

  M.update_diagnostic_config(bufnr, toggle_state)

  require("lazy.core.util").info(
    toggle_state.diagnostic_virtual_text and "Enabled virtual text" or "Disabled virtual text",
    { title = "Diagnostics" }
  )
end
...
function M.toggle_diagnostic_virtual_lines(bufnr, toggle_state)
  ...
  toggle_state.diagnostic_virtual_text = toggle_state.diagnostic_virtual_text and false

  M.update_diagnostic_config(bufnr, toggle_state)

  require("lazy.core.util").info(
    toggle_state.diagnostic_virtual_lines and "Enabled virtual lines" or "Disabled virtual lines",
    { title = "Diagnostics" }
  )
end
...
function M:on_attach()
  return function(client, bufnr)
    ...
    local keymaps = {
      ...
      client.supports_method("textDocument/codeLens") and {
        keybinding = "cr",
        action = function()
      ...
      client.supports_method("textDocument/definition")
          and { keybinding = "gd", action = "<cmd>Glance definitions<cr>", desc = "Goto Definition" }
        or {},
      ...
      client.supports_method("textDocument/signatureHelp")
          and { keybinding = "gK", action = vim.lsp.buf.signature_help, desc = "Signature Help" }
        or {},
      client.supports_method("textDocument/signatureHelp")
          and { keybinding = "<c-k>", mode = "i", action = vim.lsp.buf.signature_help, desc = "Signature Help" }
        or {},
      ...
      M.supports_format(client)
          and client.supports_method("textDocument/rangeFormatting")
          and { keybinding = "<leader>cf", mode = "v", action = "", desc = "Format Range" }
        or {},

      client.supports_method("textDocument/codeAction")
          and { keybinding = "<leader>ca", action = vim.lsp.buf.code_action, desc = "Code Action", mode = { "n", "v" } }
        or {},
      ...
      treesitter_active
          and { keybinding = "zr", action = require("ufo").openFoldsExceptKinds, desc = "Open Folds (Except Kinds)" }
        or {},
      treesitter_active
          and { keybinding = "zm", action = require("ufo").closeFoldsWith, desc = "Close Folds (Except Kinds)" }
        or {},

      M.supports_format(client) and {
        keybinding = "<leader>uf",
        action = function()
          M.toggle_format(toggle_state)
        end,
